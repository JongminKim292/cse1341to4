...
if(addBtnTarget){
  addBtnTarget.addEventListener('click',function closeAndParse(){
    if(!titleTarget.value){
      ...
      list.innerHTML = `<span><span>${titleTarget.value}</span> (<i>${dateTarget.value}</i>) - Summary : <option style="display:inline;">${summaryTarget.value}</option></span>`;
    ...
    editItem.addEventListener('click',function(){
      editPostingDialogTarget.showModal();
        ...
        rate.value = this.parentNode.firstChild.lastChild.value;
        editBtnTarget.addEventListener('click',function(){
          list.innerHTML = `<span><span>${title.value}</span> (<i>${year.value}</i>) - Summary : <option style="display:inline;">${rate.value}</option></span>`
          list.appendChild(deleteItem);
          list.appendChild(editItem);
